namespace WinFormsApp1
{
    ...
    class Warstwa
    {
        ...
        public void changeValue(List<double> Neuronvalue)
        {
            ...
            {
                throw new Exception("Niew³asciwe dane wejœciowe");
            }
    ...
    class Network
    {
        ...
        private Form1 form;
        private double maxnumbererror;
        public Network(List<int> Params, Form1 form, double maxnumbererror, double paramteruczenia=0.1) //2 2 3 ->2 neurony->2 neurony -> 3 neurony?
        {
            ...
            this.paramteruczenia = paramteruczenia;
        }
        public void ShowWages()
        {
            for (int i = 1; i < Warstwa.Count; i++)
            {
                form.ConsoleBox("Wagi Warstwa nr.{0}",i);
                for(int j = 0; j < Warstwa[i].Neurons.Count; j++)
                {
                    form.ConsoleBox("Neuron nr.{0} wagi: {1}", j, string.Join(',', Warstwa[i].Neurons[j].wagi));
                }    
                //Wagi
            }
        }
        public List<List<List<double>>> getArrayWags()
        {
            List<List<List<double>>> result = new() { };
            for (int i = 1; i < Warstwa.Count; i++)
            {
                List<List<double>> wagswarstwa = new();
                for (int j = 0; j < Warstwa[i].Neurons.Count; j++)
                {
                    List<double> wagsneuron = new();
                    for (int k = 0; k < Warstwa[i].Neurons[j].wagi.Count; k++)
                    {
                        wagsneuron.Add(Warstwa[i].Neurons[j].wagi[k]);
                    }
                    wagsneuron.Add(Warstwa[i].Neurons[j].extranumber);
                    wagswarstwa.Add(wagsneuron);
                }
                //Wagi
                result.Add(wagswarstwa);
            }
            return result;
        }

        public void LoadWagsFromTxt(string data)
        {
            //Skomplikowane Load
            var datatemp = data.Split("\r\n\r\n");
            if(datatemp.Length+1 != this.Warstwa.Count)
            {
                form.ConsoleBox("Iloœæ Warstw siê niezgadza");
                return;
            }
            for(int warstwa = 0; warstwa < datatemp.Length; warstwa++)
            {
                var neuron = datatemp[warstwa].Split("\r\n");
                if(neuron.Length != this.Warstwa[warstwa + 1].Neurons.Count)
                {
                    form.ConsoleBox("W warstwie nr.{0} Nie zgadza siê iloœæ neuronów",warstwa);
                    return;
                }
                for (int neur = 0; neur < neuron.Length; neur++)
                {
                    var wagi = neuron[neur].Split(" ");
                    double bias;
                    double.TryParse(wagi[wagi.Length - 1], out bias);

                    if (wagi.Length != this.Warstwa[warstwa + 1].Neurons[neur].wagi.Count+1)
                    {
                        form.ConsoleBox("W warstwie nr.{} w Neuronie nr.{} Nie zgadza siê iloœæ neuronów",warstwa,neur);
                    }
                    for(int waga= 0;waga< wagi.Length-1;waga++)
                    {
                        double doublewaga;
                        double.TryParse(wagi[waga], out doublewaga);
                        this.Warstwa[warstwa + 1].Neurons[neur].wagi[waga] = doublewaga;
                    }
                    this.Warstwa[warstwa + 1].Neurons[neur].extranumber = bias;

                }
            }
            /*
            foreach(var warstwa in datatemp)
            {
                var neuron = warstwa.Split("\r\n");
                
                foreach(var wags in neuron)
                {
                    var wagiarray = wags.Split(" ");
                }
            }
            */
            //Podzia³ na warstwa
        ...
        public List<double> CheckError(List<List<double>> probki, int option = 0)
        {
            ...
            for (int i = 0; i < probki.Count; i++)
            {
                ...
                //Co dalej?
                if (option == 1) form.ConsoleBox("Dla próbki {0}", string.Join(",", probki[i]));
                for (int j = 0; j < resultlast.Count; j++)
                {
                    double tempresult = Math.Abs(output[j] - resultlast[j]);
                    result.Add(tempresult);
                    if (option == 1) form.ConsoleBox("Neuron nr.{0} B³ad:{1} Wyjscie: {2}", j, tempresult, resultlast[j]);
                }
